...
### Fixed
- Added missing RTSP port mappings (8554, 8555/tcp, 8555/udp) to the generated `compose.yml`. These ports are required by go2rtc for live camera streaming in Home Assistant. Without them, Home Assistant's Frigate integration would only display a single still frame. (#55)
- Updated the optional Proxmox firewall rules to also open ports 8554 and 8555 (TCP/UDP) when the `--firewall` flag is used.
